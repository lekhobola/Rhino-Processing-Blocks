...
Fs = 50e6; % sampling frequency in KHz
%fc = 98e6;  % cut-off frequency in KHz
%t = 0:N-1;
t = -Fs/2:Fs/N:Fs/2-Fs/N;

%x1T = round(A * exp(j*2*pi*f*t/Fs));

x1T = round(A * rectpuls(t,50*(Fs/N)));%makes the actual square pulse
