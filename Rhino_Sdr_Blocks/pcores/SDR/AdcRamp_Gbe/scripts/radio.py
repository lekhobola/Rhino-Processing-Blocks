...
os.environ['PRJ_PATH'] = '/home/lekhobola/Documents/dev/research/xilinx/sdrg/AdcRamp_Gbe/Rhino_Sdr_Blocks/pcores/SDR/script'

# capture a fixed number of udp packets [total passed a argument in the program command], the output file is "AdcRamp_Gbe.pcap"
os.system('sudo tcpdump -i eno1 udp -c %s -w $PRJ_PATH/AdcRamp_Gbe.pcap' % (sys.argv[1]))

# extract udp payload in every udp packet, the output file is "Fmcomms_BIST_960khz_stream_dump.txt"
os.system('tshark -r $PRJ_PATH/AdcRamp_Gbe.pcap -T fields -e data > $PRJ_PATH/AdcRamp_Gbe.txt')

os.system('capinfos ./ADcRamp_Gbe.pcap')


hex_data = []	
with open('AdcRamp_Gbe.txt') as fp:
	with open('AdcRamp_Gbe.dat', 'wb') as f:
		for frame in fp:
